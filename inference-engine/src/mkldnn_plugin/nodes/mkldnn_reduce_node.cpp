...
void MKLDNNReduceNode::initSupportedPrimitiveDescriptors() {
    ...
    if (jit_mode) {
        ...
        if (getParentEdgeAt(REDUCE_DATA)->getShape().getStaticDims()[1] > 1) {
            if (keep_dims) {
                if (getParentEdgeAt(REDUCE_DATA)->getShape().getRank() == 4) {
                    if (mayiuse(cpu::x64::avx512_common)) {
                ...
                } else if (getParentEdgeAt(REDUCE_DATA)->getShape().getRank() == 5) {
                    if (mayiuse(cpu::x64::avx512_common)) {
                ...
                auto planar_format = MKLDNNMemory::GetPlainFormatByRank(getChildEdgeAt(0)->getShape().getRank());
                if (getParentEdgeAt(REDUCE_DATA)->getShape().getRank() == 4) {
                    if (mayiuse(cpu::x64::avx512_common)) {
