...
private:
    ...
    unsigned char cmp_flg = jit_generator::_cmp_lt_os;
...
void MKLDNNTopKNode::createPrimitive() {
    ...
    // [case 2]: otherwise use bitonic sort.
    bitonic_sort = top_k <= count_vec / 2 - 2 ? false : true;
    if (bitonic_sort) {
...
void MKLDNNTopKNode::topk_PLN(const uint8_t *in_ptr, uint8_t *out_ptr, uint8_t *out_idx_ptr,
    ...
    const float *pIn = reinterpret_cast<const float *>(in_ptr);
    std::cout << "pIn: " << std::endl;
    for (int o = 0; o < O; o++) {
        for (int a = 0; a < A; a++) {
            for (int i = 0; i < I; i++) {
                std::cout << pIn[o*A*I+a*I+i] << " ";
            }
            std::cout << std::endl;
        }
        std::cout << std::endl;
    ...
    float *pOut = reinterpret_cast<float *>(out_ptr);
    std::cout << "pOut: " << std::endl;
    for (int o = 0; o < O; o++) {
        for (int k = 0; k < top_k; k++) {
            for (int i = 0; i < I; i++) {
                std::cout << pOut[o*top_k*I+k*I+i] << " ";
            }
            std::cout << std::endl;
        }
        std::cout << std::endl;
    ...
    int *pIdx = reinterpret_cast<int *>(out_idx_ptr);
    std::cout << "pIdx: " << std::endl;
    for (int o = 0; o < O; o++) {
        for (int k = 0; k < top_k; k++) {
            for (int i = 0; i < I; i++) {
                std::cout << pIdx[o*top_k*I+k*I+i] << " ";
            }
            std::cout << std::endl;
        }
        std::cout << std::endl;
...
void MKLDNNTopKNode::topk_BLK(const uint8_t *in_ptr, uint8_t *out_ptr, uint8_t *out_idx_ptr,
    ...
    std::cout << "pIn: " << std::endl;
    for (int n = 0; n < N; n++) {
        for (int ic = 0; ic < src_dims[1]; ic++) {
            int icb = ic / blk_size;
            int ik = ic % blk_size;
            for (int d = 0; d < D; d++) {
                for (int h = 0; h < H; h++) {
                    for (int w = 0; w < W; w++) {
                        std::cout << pIn[((((n * ICB + icb) * D + d) * H + h) * W + w) * blk_size + ik]<< " ";
                    }
                    std::cout << std::endl;
                }
                std::cout << std::endl;
    ...
    std::cout << "pOut: " << std::endl;
    for (int n = 0; n < N; n++) {
        for (int oc = 0; oc < dst_dims[1]; oc++) {
            int ocb = oc / blk_size;
            int ok = oc % blk_size;
            for (int d = 0; d < D; d++) {
                //for (int h = 0; h < H; h++) {
                for (int h = 0; h < top_k; h++) {
                    for (int w = 0; w < W; w++) {
                        std::cout << pOut[((((n * OCB + ocb) * D + d) * H + h) * W + w) * blk_size + ok]<< " ";
                    }
                    std::cout << std::endl;
                }
                std::cout << std::endl;
    ...
    for (int n = 0; n < N; n++) {
        for (int oc = 0; oc < dst_dims[1]; oc++) {
            ...
            for (int d = 0; d < D; d++) {
                ...
                for (int h = 0; h < top_k; h++) {
                    for (int w = 0; w < W; w++) {
                        std::cout << pIdx[((((n * OCB + ocb) * D + d) * H + h) * W + w) * blk_size + ok]<< " ";
                    }
                    std::cout << std::endl;
                }
                std::cout << std::endl;
