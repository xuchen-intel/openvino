...
struct jit_topk_config_params {
    ...
    int blk_stride;     // stride of channel blocks at the same space coordinate, only used in blocked layout with channel-topk
};
...
private:
    ...
    void top1_axis(const float* src_data, float* dst_data, int32_t* dst_idx,
                   InferenceEngine::SizeVector in_dims, std::function<float(float, float)> compare);
