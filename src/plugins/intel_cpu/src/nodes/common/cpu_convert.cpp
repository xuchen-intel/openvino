...
[[maybe_unused]] static int8_t get_u1(const uint8_t& val, uint8_t shift) {
    ...
    std::cout << "******** val & (0x1 << shift): " << static_cast<int>(val & (0x1 << shift)) << std::endl;
    std::cout << "******** (val & (0x1 << shift)) >> shift: " << static_cast<int>((val & (0x1 << shift)) >> shift) << std::endl;
#endif
...
#define INTEL_CPU_CVT_FROM_2BIT_LIST INTEL_CPU_CVT(u2, f32), INTEL_CPU_CVT(u2, f16), INTEL_CPU_CVT(u2, bf16)
