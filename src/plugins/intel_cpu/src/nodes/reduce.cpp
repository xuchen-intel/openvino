...
void Reduce::createPrimitive() {
    ...
    if (mayiuse(cpu::x64::avx512_common)) {
        reduce_kernel.reset(new jit_uni_reduce_kernel_f32<cpu::x64::avx512_common>(reduce_jcp));
        blk_size = 16;
    } else if (mayiuse(cpu::x64::avx2)) {
        ...
        for (size_t ib = 0; ib < IB; ib++) {
            ...
            if (!ReduceC && !ReduceD && ReduceW) {
                ...
                parallel_for(IC, [&](size_t ic) {
                    ...
                    parallel_for(IW / blk_size, [&](size_t ibw){
                        size_t obw = ibw;
                        reduce_kernel_process(in_ptr_nc + ibw * blk_size * src_data_size, out_ptr_nc + obw * blk_size * data_size,
                                              blk_size, 0, ID * IH);
                    });
                    size_t tail_start = IW / blk_size * blk_size;
                    reduce_kernel_process(in_ptr_nc + tail_start * src_data_size, out_ptr_nc + tail_start * data_size,
...
void Reduce::reduce_BLK(const uint8_t *in_ptr, uint8_t *o_ptr) {
    ...
    for (size_t ib = 0; ib < IB; ib++) {
        ...
        if (!ReduceC && !ReduceD && ReduceH && ReduceW) {
            ...
            if (input_prec != data_prec || getAlgorithm() == Algorithm::ReduceL2 ||
