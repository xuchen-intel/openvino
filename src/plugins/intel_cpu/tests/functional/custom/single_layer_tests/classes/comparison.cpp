...
std::string ComparisonLayerCPUTest::getTestCaseName(const testing::TestParamInfo<ComparisonLayerCPUTestParamSet> &obj) {
    ...
    utils::ComparisonTypes comparisonType;
    ov::element::Type netPrecision, inPrecision, outPrecision;
    CPUTestUtils::CPUSpecificParams cpuParams;
    bool enforceSnippets;
    std::tie(inputShapes, comparisonType, netPrecision, inPrecision, outPrecision, cpuParams, enforceSnippets) = obj.param;
    std::ostringstream result;
    result << comparisonNames[comparisonType] << "_";
...
void ComparisonLayerCPUTest::SetUp() {
    ...
    CPUTestUtils::CPUSpecificParams cpuParams;
    bool enforceSnippets;
    std::tie(inputShapes, comparisonType, netPrecision, inPrecision, outPrecision, cpuParams, enforceSnippets) = this->GetParam();
    ...
    auto comparison = utils::make_comparison(param1, param2, comparisonType);
    comparison->get_rt_info() = getCPUInfo();
    function = std::make_shared<ov::Model>(ov::NodeVector{comparison}, ov::ParameterVector{param1, param2}, "Comparison");
}

std::string ComparisonLayerCPUTest::getPrimitiveType() {
    return "jit";
}
...
const std::vector<utils::ComparisonTypes>& comparisonTypes() {
    static std::vector<utils::ComparisonTypes> types {
        EQUAL,
    };
