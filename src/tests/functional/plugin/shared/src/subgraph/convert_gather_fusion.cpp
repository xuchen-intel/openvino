...
void ConvertGatherFusionTest::SetUp() {
    ...
    // so the Convert stays in front of Gather, which prevents FuseGatherAndConvert. So,
    // a new FuseConvertAndGather pass is needed.
    ...
    // Constant(bf16) + Convert(f32) into a single Constant(f32).
